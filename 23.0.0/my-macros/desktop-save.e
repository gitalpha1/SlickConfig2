...
_str save_file(_str filename,_str options)
{
   ...
   _str pa = 'C:\Users\GP\Google Drive\slick\desktop\backups\' :+ strip_filename(filename,'DN');
   if (!path_exists(pa)) {
   ...
   pa = 'C:\Users\GP\OneDrive\slick\desktop\backups' :+ strip_filename(filename,'DN');
   if (!path_exists(pa)) {
      int result = make_path(pa);
      if (result) {
...
_command gui_save_config(_str save_immediate='') name_info(','VSARG2_EDITORCTL)
{
   ...
   if ( status == 0 ) {
      ...
      _str fn = 'C:/SlickConfig/temp/' :+ p1;;

      _mdi.export_options('-a  ' :+ fn);
      copy_file(fn, "C:/Users/GP/Google Drive/slick/desktop/backup-config/" :+ p1);
      delete_file(fn);
   }
   return status;
}
